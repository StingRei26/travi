...
export default async function TraviDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const travi = mockTraviis.find((t) => t.id === id);
  if (!travi) notFound();
      ...
      >
        ...
        <div style={{ maxWidth: "1000px", margin: "0 auto", position: "relative", zIndex: 1 }}>
          ...
          <div style={{ display: "flex", gap: "28px", marginTop: "32px", flexWrap: "wrap" }}>
            <StatPill icon="📅" label={`${travi.startDate} → ${travi.endDate}`} />
            <StatPill icon="📍" label={`${travi.stops.length} stops`} />
            <StatPill icon="👁" label={`${travi.stats.views.toLocaleString()} views`} />
            <StatPill icon="💬" label={`${travi.stats.comments} comments`} />
          </div>
        </div>
      </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
                {travi.stops.map((stop, i) => (
                  <div key={stop.id} style={{ display: "flex", gap: "20px", position: "relative" }}>
                    ...
                    {/* Card */}
                    <div style={{ flex: 1, backgroundColor: "#ffffff", borderRadius: "16px", border: "1px solid #e7e5e0", padding: "20px 24px", boxShadow: "0 2px 12px rgba(0,0,0,0.04)" }}>
                      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "8px", gap: "12px" }}>
                        <div>
                          <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
                            <span style={{ fontSize: "10px", fontWeight: "700", padding: "2px 8px", borderRadius: "100px", backgroundColor: typeColors[stop.type] + "20", color: typeColors[stop.type] }}>
...
function ActionBtn({ icon, label, accent }: { icon: React.ReactNode; label: string; accent?: boolean }) {
  return (
    <button style={{ display: "flex", alignItems: "center", gap: "8px", padding: "10px 16px", borderRadius: "10px", border: accent ? "none" : "1px solid rgba(255,255,255,0.25)", backgroundColor: accent ? "linear-gradient(135deg, #c9a84c, #e8c96a)" : "rgba(255,255,255,0.1)", background: accent ? "linear-gradient(135deg, #c9a84c, #e8c96a)" : "rgba(255,255,255,0.1)", color: accent ? "#0f1729" : "#ffffff", fontWeight: "600", fontSize: "14px", cursor: "pointer", backdropFilter: "blur(8px)", whiteSpace: "nowrap" }}>
      {icon} {label}
    </button>
  );
}
